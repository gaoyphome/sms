...
public class NumberStringUtil {
	...
	/**
	 * 
	 * 功能：把字符串转换成BigDecimal数字
	 */
	public static BigDecimal valueOfBigDecimal(String str) {
		return new BigDecimal(NumberStringUtil.isNumeric(str) ? str : "0");
	}
	 ...
	 * @return byte[]
	 */
	public static byte[] highToLow4(int n) {
		byte[] b = new byte[4];
		b[3] = (byte) (n & 0xff);
		b[2] = (byte) (n >> 8 & 0xff);
		b[1] = (byte) (n >> 16 & 0xff);
	 ...
	 * @return byte[]
	 */
	public static byte[] highToLow2(int n) {
		byte[] b = new byte[2];
		b[1] = (byte) (n & 0xff);
		b[0] = (byte) (n >> 8 & 0xff);
		return b;
	...
	/**
	 * 将byte数组转为int,低字节在前，高字节在后
	 * @param b
	 * @return
	 */
	public static int lowToHigh(byte[] b) {
	...
	/**
	 * Convert hex string to byte[]  low to high
	 * 
	 * @param hexString
	 *            the hex string
	 * @return byte[]
	 */
	public static byte[] hexStringToBytesLowToHigh(String hexString) {
		if (hexString == null || hexString.equals("")) {
			return null;
		}
		hexString = hexString.toUpperCase();
		...
		for (int i = 0, j= length-1; i < length; i++, j--) {
			int pos = i * 2;
			d[j] = (byte) (charToByte(hexChars[pos]) << 4 | charToByte(hexChars[pos + 1]));
		}
		return d;
	}
	...
	public static String getStrAd(String str, int length, int flag, char c, String encode) {
		if (flag == LPAN) {
			return NumberStringUtil.addLeftChar(NumberStringUtil.getByteLengthStr(str, length, encode), length, c,
					encode);
		} else {
			return NumberStringUtil.addRightChar(NumberStringUtil.getByteLengthStr(str, length, encode), length, c,
					encode);
		}
	}
	...
	public static String rPadEightZero80(String str) {
		if (str == null) {
	...
	public static void main(String[] args) {
		/*String str = "100";
		bytesToHexString(str.getBytes());
		
		System.out.println(bytesToHexString(str.getBytes()));*/
		
		String token = "09344020054579449311";
		System.out.println(removeLeftChar(token, '0'));
		
		String fieldHexStr = Integer.toHexString(Integer.parseInt("1"));
		System.out.println(fieldHexStr);
	}
}

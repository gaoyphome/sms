...
public class CRC16 {
    ...
    public static String calcCrc16(byte[] data, int len) {
        ...
        for (j = 0;j < len; j++){
            ...
            for (i = 0x80; i != 0; i /= 2) {
                ...
                if ((t&i) != 0) crc ^= 0x1021; /* 再加上本位的CRC */
            }

        }

        outcrc[0] = (byte)(crc / 256);
        outcrc[1] = (byte)(crc % 256);
        outcrc[2] = (byte)0xff;
        outcrc[3] = (byte)0xff;

        return HexCodec.hexEncode(outcrc).substring(0,4);
    }
	...
	public static int CRC_POLY_DEFAULT = 0xC0C1; //生成多项式:X^15 + X^14 + X^7 + X^6 + 1
	// crc_poly
	...
	public static String CRC_16(byte[] bytes) {
		...
		crc &= 0xffff;
		return HexCodec.hexEncode(intToBytes2(crc)).substring(0,4);
	}
...
//		int crc = CRC16.calcCrc16(new byte[] { 0x2F, 0x05, 0x00, 0x03, (byte) 0xff, (byte) 0xff,(byte) 0xff });
...
//		System.out.println(String.format("%04x", crc));
		...
		System.out.println(CRC16.CRC_16(bytes));
		System.out.println(CRC16.CRC_16(HexCodec.hexDecode("1234567890")));
	}
}
